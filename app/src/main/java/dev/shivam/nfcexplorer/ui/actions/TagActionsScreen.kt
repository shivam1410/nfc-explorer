...
private fun DraftEditor(
    ...
    SectionCard(
        ...
        when (draft.type) {
            ActionType.LAUNCH_APP -> OutlinedTextField(
                value = draft.packageName,
                onValueChange = { onDraftChange(draft.copy(packageName = it)) },
                label = { Text(stringResource(R.string.actions_package)) },
...
private fun summarise(action: TagAction): String = when (action) {
    ...
    is TagAction.MediaCommand -> stringResource(R.string.actions_summary_media, action.key.name)
}
